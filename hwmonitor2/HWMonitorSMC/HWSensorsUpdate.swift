...
extension PopoverViewController {
  @objc func updateCPUSensors() {
    if self.initiated && (self.CPUNode != nil) {
      var newRead : [HWMonitorSensor] = self.sensorScanner.get_CPU_GlobalParameters()
  ...
  @objc func updateGPUSensors() {
    if self.initiated && (self.GPUNode != nil) {
      var newRead : [HWMonitorSensor] = self.sensorScanner.getSMCGPU()
  ...
  @objc func updateMotherboardSensors() {
    if self.initiated && (self.MOBONode != nil) {
      let newRead = self.sensorScanner.getMotherboard()
  ...
  @objc func updateFanSensors() {
    if self.initiated && (self.FansNode != nil) {
      let newRead = self.sensorScanner.getFans()
  ...
  @objc func updateRAMSensors() {
    if self.initiated && (self.RAMNode != nil) {
      let newRAM = self.sensorScanner.getMemory()
  ...
  @objc func updateBatterySensors() {
    if self.initiated && (self.batteriesNode != nil) {
      let newBattery = self.sensorScanner.getBattery()
  ...
  func updateStatuBar() {
      ...
      if let sensor = node.sensorData?.sensor {
        if sensor.favorite {
          if components.count <= 12 {
            components.append(sensor.stringValue)
          } else {
    ...
    if useGadget {
      ...
      AppSd.statusItem.length = (AppSd.statusItem.button?.intrinsicContentSize.width)! + 12
    }
