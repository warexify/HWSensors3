...
public:
  ...
  virtual UInt8     ioRead8(   UInt16 offset, IOMemoryMap * map = 0 );
	...
	virtual IOReturn	registerInterrupt(int source,
                                      OSObject *target,
  ...
  virtual void      loadKeysFromClover(IOService *platform);
	...
	virtual FakeSMCKey	*addKeyWithValue(const char *name,
                                       const char *type,
                                       unsigned char size,
                                       const void *value);
  
	virtual FakeSMCKey	*addKeyWithHandler(const char *name,
                                         const char *type,
                                         unsigned char size,
                                         IOService *handler);
  
  virtual void        saveKeyToNVRAM(FakeSMCKey *key);
