...
OSDefineMetaClassAndStructors(Andigilog, IOService) // FakeSMCPlugin)
...
bool Andigilog::start(IOService *provider) {
  ...
  for (i = 0; i < sizeof(addrs) / sizeof(addrs[0]); i++) {
    if (!i2cNub->ReadI2CBus(addrs[i], &(cmd = ASC7621_VID_REG), sizeof(cmd), &data, sizeof(data))) {
      if ((data == ASC7621_VID) || (data == EMC6D103_VID) || (data == ADT7490_VID)) {
        if (!i2cNub->ReadI2CBus(addrs[i], &(cmd = ASC7621_PID_REG), sizeof(cmd), &data, sizeof(data)) &&
           (data == ASC7621_PID ||
            data == ASC7621A_PID ||
            data == ADT7490_PID ||
            data == EMC6D103_PID)) {
             Asc7621_addr = addrs[i];
             IOPrint("aSC PID=0x%x attached at 0x%x.\n", data, Asc7621_addr);
             break;
           } else {
             IOPrint("found chip PID %x\n", data);
           }
      } else {
...
IOReturn Andigilog::callPlatformFunction(const OSSymbol *functionName,
      ...
      if (key[0] == 'T' || (key[0] == KEY_FORMAT_FAN_SPEED[0] &&
        ...
        if (key[1] == KEY_FAN_FORCE[1]) {
          ...
          return kIOReturnSuccess;
                   ...
                   (key[3] == KEY_FORMAT_FAN_MIN_SPEED[4] ||
                    key[3] == KEY_FORMAT_FAN_MAX_SPEED[4])) {
